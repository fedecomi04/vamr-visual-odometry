"""
Main script with Ground Plane Scale Correction + Loop Closure (sidecar).

Strict requirement:
- KLT + ground-scale behavior must match `src/main_with_ground_scale.py` exactly when
  `ENABLE_LOOP_CLOSURE = False`.
- Loop closure is added on top and must not interfere with the VO update path unless a
  loop correction is applied.
"""
...
# Loop-closure sidecar imports (SIFT-only mapping + LC).
from sift_mapping.sift_mapper import SIFTMapper

from loop_closing.correction import compute_loop_sim3_measurement
...
# Phase 3+: SIFT-only loop closure sidecar (disable to benchmark VO-only).
...
def _max_consecutive_jump(traj_xyz: list, frame_idxs: list[int]) -> tuple[float, int]:
    pts = np.asarray(traj_xyz, dtype=np.float64).reshape(-1, 3)
    if pts.shape[0] < 2:
        return 0.0, int(frame_idxs[-1]) if frame_idxs else -1
    d = np.linalg.norm(np.diff(pts, axis=0), axis=1)
    j = int(np.argmax(d))
    max_jump = float(d[j])
    # Jump between frames (j) and (j+1): report the latter frame index.
    idx = int(frame_idxs[j + 1]) if (j + 1) < len(frame_idxs) else -1
    return max_jump, idx
...
# --- Pose history for world-reset debugging (per-frame world->camera SE3) ---
poses_wc: list[np.ndarray] = []
trajectory_frame_idxs: list[int] = []
...
trajectory_frame_idxs.append(int(bootstrap_frames[1]))

# --- Loop closing sidecar state (SIFT-only) ---
if ENABLE_LOOP_CLOSURE:
    sift_mapper = SIFTMapper(K=intrinsics.K, max_kf_features=800)
    descriptor_db = DescriptorDatabase(debug=LC_DEBUG)
    ...
    # Seed with bootstrap pose/image: will create the first SIFT keyframe (and run SIFT once).
    kf0 = sift_mapper.process_frame(
        frame_idx=int(bootstrap_frames[1]),
        image_gray=state.image,
        R_wc=state.R,
        t_wc=state.t,
    )
    if kf0 is not None:
        n_des = 0 if kf0.descriptors is None else int(len(kf0.descriptors))
        dtyp = None if kf0.descriptors is None else str(np.asarray(kf0.descriptors).dtype)
        n_lm = int(np.count_nonzero(np.asarray(kf0.landmark_ids, dtype=np.int64) != -1))
        frac = float(n_lm / max(1, len(kf0.landmark_ids)))
        print(
            f"[LC-KF] frame={kf0.frame_idx} kp={len(kf0.keypoints)} des={n_des} dtype={dtyp} lm={n_lm} frac={frac:.2f}"
        )
        descriptor_db.add_keyframe(kf0)
        pose_graph.add_node_from_keyframe(kf0)
        prev_sift_kf = kf0
        last_keyframe_features = int(0 if kf0.descriptors is None else len(kf0.descriptors))
else:
    loops_detected = 0
    last_loop_text = None
    last_keyframe_features = 0
...
for i in range(bootstrap_frames[1] + 1, dataset.last_frame + 1):
    ...
    # Pose history (world->camera) for world-reset application + diagnostics.
    ...
    trajectory_frame_idxs.append(int(i))
    ...
    # --- Loop closure sidecar (SIFT-only; does not touch VO pose unless a correction is applied) ---
    if ENABLE_LOOP_CLOSURE:
        kf_sift = sift_mapper.process_frame(i, state.image, state.R, state.t)
        if kf_sift is not None:
            # Only SIFT float descriptors in this mode.
            last_keyframe_features = int(0 if kf_sift.descriptors is None else len(kf_sift.descriptors))
            n_des = 0 if kf_sift.descriptors is None else int(len(kf_sift.descriptors))
            dtyp = None if kf_sift.descriptors is None else str(np.asarray(kf_sift.descriptors).dtype)
            n_lm = int(np.count_nonzero(np.asarray(kf_sift.landmark_ids, dtype=np.int64) != -1))
            frac = float(n_lm / max(1, len(kf_sift.landmark_ids)))
            print(
                f"[LC-KF] frame={kf_sift.frame_idx} kp={len(kf_sift.keypoints)} des={n_des} dtype={dtyp} lm={n_lm} frac={frac:.2f}"
            )
            descriptor_db.add_keyframe(kf_sift)
            pose_graph.add_node_from_keyframe(kf_sift)
            if prev_sift_kf is not None:
                pose_graph.add_tree_edge(prev_sift_kf, kf_sift)
            prev_sift_kf = kf_sift
            ...
            if loop_candidate is None:
                print(
                    f"[SIFTLC] frame={int(i)} kp={len(kf_sift.keypoints)} des={n_des} dtype={dtyp} "
                    f"matches=0 E_inl=0 Sim3_pairs=0 Sim3_inl=0 ACCEPT_LOOP=0"
                )
            else:
                loops_detected += 1
                ...
                class _LCState:
                    def __init__(self, landmarks_3d: np.ndarray):
                ...
                if LC_DEBUG:
                    ...
                    idx_m = idx_m[:n]
                    inl = inl[:n]
                    lm_q = np.asarray(q_kf.landmark_ids, dtype=np.int64).reshape(-1)
                    lm_m = np.asarray(m_kf.landmark_ids, dtype=np.int64).reshape(-1)
                    shared = 0
                    for k in np.where(inl)[0]:
                        ...
                        if iq < 0 or iq >= lm_q.size or im < 0 or im >= lm_m.size:
                            continue
                        if int(lm_q[iq]) != -1 and int(lm_m[im]) != -1:
                            shared += 1
                    print(f"[LC-Pairs] inlier_matches={int(np.count_nonzero(inl))} with_3d_both={int(shared)}")

                meas, mstats = compute_loop_sim3_measurement(
                    loop_candidate,
                    pose_graph,
                    _LCState(sift_mapper.map.landmarks_3d),
                    sim3_thresh=0.5,
                    max_iters=2000,
                    min_pairs=30,
                    min_inliers=20,
                ...
                if meas is not None:
                    ...
                    rot_deg = float(
                        np.degrees(np.linalg.norm(cv2.Rodrigues(R_rel)[0].reshape(3)))
                    )
                    t_norm = float(
                        np.linalg.norm(np.asarray(t_rel, dtype=np.float64).reshape(3))
                    )
                ...
                if meas is not None:
                    pose_graph.add_loop_edge(q_kf, m_kf, R_rel, t_rel, s_rel)
                    ...
                    if WORLD_RESET_AFTER_LOOP:
                        ...
                        s_w, R_w, t_w = decompose_sim3_matrix(S_W)
                        ...
                        apply_world_correction_to_keyframes(sift_mapper.map.keyframes, S_W)
                        pose_graph.reset_nodes_from_keyframes(sift_mapper.map.keyframes)
                        ...
                        sift_mapper.map.apply_sim3_inplace(S_W)
                        ...
                        if getattr(state, "trajectory", None) is not None and len(state.trajectory) > 0:
                            ...
                            state.trajectory = [traj_pts[ii] for ii in range(traj_pts.shape[0])]
                        ...
                        rot_deg = float(
                            np.degrees(
                                np.arccos(np.clip((np.trace(R_w) - 1.0) / 2.0, -1.0, 1.0))
                            )
                        )
                        trans = float(np.linalg.norm(np.asarray(t_w, dtype=np.float64).reshape(3)))
                        print(f"[WorldReset] delta_pose: trans={trans:.3f}m rot={rot_deg:.2f}deg scale={float(s_w):.3f}")
                        max_jump, at_frame = _max_consecutive_jump(state.trajectory, trajectory_frame_idxs)
                        print(f"[WorldReset] max_consecutive_jump={max_jump:.3f} at frame={int(at_frame)}")
                        print(f"[WorldReset] applied S_W: pose_jump ||C_new-C_old||={jump:.3f}")
                ...
                e_inl = int(np.count_nonzero(np.asarray(loop_candidate.inlier_mask, dtype=bool)))
                acc = int(1 if (meas is not None and mstats.accepted) else 0)
                print(
                    f"[SIFTLC] frame={int(i)} kp={len(kf_sift.keypoints)} des={n_des} dtype={dtyp} "
                    f"matches={int(len(loop_candidate.idx_query))} E_inl={e_inl} "
                    f"Sim3_pairs={int(mstats.pairs)} Sim3_inl={int(mstats.inliers)} ACCEPT_LOOP={acc}"
                )
    ...
    if UI_activate:
        update_ui(
            ...
            keyframes_count=(len(sift_mapper.map.keyframes) if ENABLE_LOOP_CLOSURE else 0),
            loop_edges_count=(pose_graph.loop_edges_count if ENABLE_LOOP_CLOSURE else 0),
            db_features_count=(descriptor_db.total_descriptors if ENABLE_LOOP_CLOSURE else 0),
...
if dataset.ground_truth is not None:
    ...
    try:
        ...
        if UI_activate and rte_results is not None:
            ...
            _update_ui_final(
                ...
                axis_limits=state.axis_limits,
                rte_errors=rte_results["per_pair_errors"],
                ground_points_px=ground_points_px,
                full_trajectory_gt=dataset.ground_truth,
            )
